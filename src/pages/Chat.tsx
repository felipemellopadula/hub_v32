  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    const fileData = await Promise.all(
      currentFiles.map(async (file) => {
        // Se for um PDF, envie apenas o texto extraído e não o base64
        if (file.type === "application/pdf") {
          return {
            name: file.name,
            type: file.type,
            data: "", // Envia uma string vazia para 'data', economizando espaço
            pdfContent: processedPdfs.get(file.name) || "",
          }
        }
        // Para outros tipos de arquivo (imagens), mantenha a lógica do base64
        return {
          name: file.name,
          type: file.type,
    ...
    try {
      ...
      typingIntervalRef.current = setInterval(() => {
        if (charIndex < fullBotText.length) {
          setMessages((prev) =>
            prev.map((msg) =>
              msg.id === botMessageId ? { ...msg, content: fullBotText.slice(0, charIndex + 1) } : msg,
            ),
          )
          charIndex++
        } else {
        ...
        try {
          ...
          if (result.success) setProcessedPdfs((prev) => new Map(prev).set(file.name, result.content || ""))
            ...
            toast({
              ...
              description: result.error || `Falha em ${file.name}.`,
              variant: "destructive",
            })
        } catch (error) {
          toast({
        ...
        <main className="flex-1 flex flex-col bg-background">
                          ...
                          <div className="space-y-3">
                            {message.reasoning && (
                              <div className="border-b border-border/50 pb-2">
                                ...
                                {expandedReasoning[message.id] && (
                                  <div className="mt-2 text-xs opacity-80 bg-background/50 rounded p-2 whitespace-pre-wrap overflow-hidden">
                                    {message.reasoning}
                                  </div>
                                )}
                              </div>
          ...
          {showScrollToBottom && (
            <Button
              ...
              variant="outline"
              size="icon"
              className="absolute bottom-24 right-6 h-10 w-10 rounded-full shadow-lg z-20 bg-transparent"
            >
