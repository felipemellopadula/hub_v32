...
export default function SettingsStats({ planLabel, tokensRemaining, cycleStart, cycleEnd, nextReset }: SettingsStatsProps) {
  const used = Math.max(0, TOTAL_TOKENS - (tokensRemaining || 0));
  const percent = Math.min(100, Math.max(0, (used / TOTAL_TOKENS) * 100));
